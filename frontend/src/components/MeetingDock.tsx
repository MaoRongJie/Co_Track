    ...
    <div className="relative overflow-hidden rounded-xl border border-slate-200 bg-slate-900">
      ...
      <div className="absolute bottom-0 left-0 right-0 flex items-center justify-between bg-black/60 px-2 py-1 text-[11px] text-white">
        <div className="flex items-center gap-1">
          ...
          {handRaised ? <span className="rounded bg-amber-500/80 px-1 py-0.5">举手</span> : null}
          {speakGranted ? <span className="rounded bg-emerald-500/80 px-1 py-0.5">可发言</span> : null}
        </div>
        <div className="flex items-center gap-1">
    ...
    <aside className="fixed bottom-4 right-4 z-50 w-[380px] max-w-[94vw] overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl">
      <div className="flex items-center justify-between border-b border-slate-200 bg-slate-50 px-4 py-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-slate-800">会议</span>
          <span className="rounded-full bg-slate-100 px-2 py-0.5 text-[10px] text-slate-600">
            {connecting ? '连接中' : joined ? '在线' : '离线'}
          </span>
        </div>
        <button
          type="button"
        ...
        <div className="space-y-3 p-3">
          ...
          {isHost && pendingSpeakRequests.length > 0 ? (
            <div className="rounded-lg border border-blue-200 bg-blue-50 p-2">
              <p className="mb-1 text-xs font-semibold text-blue-700">发言请求</p>
